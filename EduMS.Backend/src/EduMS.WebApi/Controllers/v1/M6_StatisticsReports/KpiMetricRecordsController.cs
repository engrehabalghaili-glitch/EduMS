...
[Authorize]
public class KpiMetricRecordsController : ControllerBase
{
    private readonly IMediator _mediator;

    public KpiMetricRecordsController(IMediator mediator)
    {
        _mediator = mediator;
    }
    ...
    {
        var result = await _mediator.Send(new CalculateLiveKpiMetricRecordQuery { SchoolId = schoolId });
    ...
    {
        var result = await _mediator.Send(new GetKpiMetricRecordSnapshotQuery { Id = id });
