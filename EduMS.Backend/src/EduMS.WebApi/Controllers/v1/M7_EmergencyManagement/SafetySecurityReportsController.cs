...
[Authorize]
public class SafetySecurityReportsController : ControllerBase
{
    private readonly IMediator _mediator;

    public SafetySecurityReportsController(IMediator mediator)
    {
        _mediator = mediator;
    }
    ...
    {
        var result = await _mediator.Send(new GetAllSafetySecurityReportsQuery());
    ...
    {
        var result = await _mediator.Send(new GetSafetySecurityReportByIdQuery { Id = id });
    ...
    {
        var id = await _mediator.Send(new CreateSafetySecurityReportCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await _mediator.Send(new UpdateSafetySecurityReportCommand { Dto = dto });
    ...
    {
        var result = await _mediator.Send(new DeleteSafetySecurityReportCommand { Id = id });
