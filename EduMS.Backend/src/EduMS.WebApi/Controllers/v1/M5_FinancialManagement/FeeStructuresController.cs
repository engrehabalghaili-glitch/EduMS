...
public class FeeStructuresController : ControllerBase
{
    ...
    {
        var result = await _mediator.Send(new GetAllFeeStructuresQuery());
    ...
    {
        var result = await _mediator.Send(new GetFeeStructureByIdQuery { Id = id });
    ...
    {
        var id = await _mediator.Send(new CreateFeeStructureCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await _mediator.Send(new UpdateFeeStructureCommand { Dto = dto });
    ...
    {
        var result = await _mediator.Send(new DeleteFeeStructureCommand { Id = id });
