...
[Authorize]
public class StudentLibraryBorrowingLogsController : ControllerBase
{
    private readonly IMediator _mediator;

    public StudentLibraryBorrowingLogsController(IMediator mediator)
    {
        _mediator = mediator;
    }
    ...
    {
        var result = await _mediator.Send(new GetAllStudentLibraryBorrowingLogsQuery());
    ...
    {
        var result = await _mediator.Send(new GetStudentLibraryBorrowingLogByIdQuery { Id = id });
    ...
    {
        var id = await _mediator.Send(new CreateStudentLibraryBorrowingLogCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await _mediator.Send(new UpdateStudentLibraryBorrowingLogCommand { Dto = dto });
    ...
    {
        var result = await _mediator.Send(new DeleteStudentLibraryBorrowingLogCommand { Id = id });
