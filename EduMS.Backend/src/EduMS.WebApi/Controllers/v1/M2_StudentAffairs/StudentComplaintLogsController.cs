...
[Authorize]
public class StudentComplaintLogsController : ControllerBase
{
    private readonly IMediator _mediator;

    public StudentComplaintLogsController(IMediator mediator)
    {
        _mediator = mediator;
    }
    ...
    {
        var result = await _mediator.Send(new GetAllStudentComplaintLogsQuery());
    ...
    {
        var result = await _mediator.Send(new GetStudentComplaintLogByIdQuery { Id = id });
    ...
    {
        var id = await _mediator.Send(new CreateStudentComplaintLogCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await _mediator.Send(new UpdateStudentComplaintLogCommand { Dto = dto });
    ...
    {
        var result = await _mediator.Send(new DeleteStudentComplaintLogCommand { Id = id });
