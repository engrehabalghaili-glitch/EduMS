...
public class JobApplicantsController : ControllerBase
{
    ...
    {
        var result = await _mediator.Send(new GetAllJobApplicantsQuery());
    ...
    {
        var result = await _mediator.Send(new GetJobApplicantByIdQuery { Id = id });
    ...
    {
        var id = await _mediator.Send(new CreateJobApplicantCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await _mediator.Send(new UpdateJobApplicantCommand { Dto = dto });
    ...
    {
        var result = await _mediator.Send(new DeleteJobApplicantCommand { Id = id });
