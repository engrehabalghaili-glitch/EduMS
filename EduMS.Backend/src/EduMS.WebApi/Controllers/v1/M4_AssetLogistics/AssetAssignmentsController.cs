...
public class AssetAssignmentsController : ControllerBase
{
    ...
    {
        var result = await _mediator.Send(new GetAllAssetAssignmentsQuery());
    ...
    {
        var result = await _mediator.Send(new GetAssetAssignmentByIdQuery { Id = id });
    ...
    {
        var id = await _mediator.Send(new CreateAssetAssignmentCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await _mediator.Send(new UpdateAssetAssignmentCommand { Dto = dto });
    ...
    {
        var result = await _mediator.Send(new DeleteAssetAssignmentCommand { Id = id });
