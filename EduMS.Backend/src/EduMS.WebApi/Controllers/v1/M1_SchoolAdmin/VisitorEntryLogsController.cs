...
[Authorize]
public class VisitorEntryLogsController : ControllerBase
{
    private readonly IMediator _mediator;

    public VisitorEntryLogsController(IMediator mediator)
    {
        _mediator = mediator;
    }
    ...
    {
        var result = await _mediator.Send(new GetAllVisitorEntryLogsQuery());
    ...
    {
        var result = await _mediator.Send(new GetVisitorEntryLogByIdQuery { Id = id });
    ...
    {
        var id = await _mediator.Send(new CreateVisitorEntryLogCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await _mediator.Send(new UpdateVisitorEntryLogCommand { Dto = dto });
    ...
    {
        var result = await _mediator.Send(new DeleteVisitorEntryLogCommand { Id = id });
