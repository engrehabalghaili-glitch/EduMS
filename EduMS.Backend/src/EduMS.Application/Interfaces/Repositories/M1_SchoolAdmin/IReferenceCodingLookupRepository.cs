...
public interface IReferenceCodingLookupRepository : IGenericRepository<ReferenceCodingLookup>
{
    ...
    Task<bool> IsCodeKeyUniqueAsync(long? schoolId, string codeType, string codeKey, long? excludeId = null);
    
    // 2. Global vs Local lookups
    // جلب الرموز المرجعية العامة (Global System Codes)
    Task<IEnumerable<ReferenceCodingLookup>> GetSystemCodesAsync(string codeType);
    
    // جلب الرموز المرجعية الخاصة بمدرسة معينة مع دمجها بالرموز العامة (اختياري)
    Task<IEnumerable<ReferenceCodingLookup>> GetCodesForSchoolAsync(long schoolId, string codeType, bool includeSystemCodes = true);
    
    // 3. Hierarchy 
    // جلب الرموز الفرعية (Child Codes) التابعة لرمز أب
    Task<IEnumerable<ReferenceCodingLookup>> GetChildCodesAsync(long parentCodeId);
}
