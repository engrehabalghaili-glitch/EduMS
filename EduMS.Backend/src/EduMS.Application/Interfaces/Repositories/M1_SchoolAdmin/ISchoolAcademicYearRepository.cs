...
public interface ISchoolAcademicYearRepository : IGenericRepository<SchoolAcademicYear>
{
    ...
    Task<bool> IsYearCodeUniqueAsync(long schoolId, string yearCode, long? excludeId = null);
    
    // 2. Status & Current Year
    // جلب العام الدراسي الحالي للمدرسة
    Task<SchoolAcademicYear?> GetCurrentAcademicYearAsync(long schoolId);
    
    // جلب الأعوام الدراسية النشطة
    Task<IEnumerable<SchoolAcademicYear>> GetOpenAcademicYearsAsync(long schoolId);
    
    // 3. Date queries
    // جلب العام الدراسي الذي يشمل تاريخاً معيناً
    Task<SchoolAcademicYear?> GetAcademicYearByDateAsync(long schoolId, DateTime date);
}
