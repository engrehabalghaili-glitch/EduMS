...
public interface IVisitorEntryLogRepository : IGenericRepository<VisitorEntryLog>
{
    ...
    Task<IEnumerable<VisitorEntryLog>> GetActiveVisitorsAsync(long schoolId);
    
    // 2. Search & History
    // البحث عن زائر برقم الهوية أو جواز السفر
    Task<IEnumerable<VisitorEntryLog>> GetVisitorHistoryByIdentityAsync(long schoolId, string nationalIdOrPassport);
    
    // جلب الزيارات التي استضافها موظف معين
    Task<IEnumerable<VisitorEntryLog>> GetVisitsByHostEmployeeAsync(long hostEmployeeId);
    
    // 3. Security
    // جلب الزوار الذين تم وسمهم بعلامة تحذير (Flagged) لأسباب أمنية
    Task<IEnumerable<VisitorEntryLog>> GetFlaggedVisitorsAsync(long schoolId);
}
