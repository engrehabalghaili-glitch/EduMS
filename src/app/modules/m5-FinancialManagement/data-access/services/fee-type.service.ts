...
export class FeeTypeService {
  ...
  private readonly baseUrl = this.apiConfig.getEndpoint('M5_FinancialManagement', 'fee-types');

  getAll(): Observable<FeeType[]> {
    return this.http.get<FeeType[]>(`${this.baseUrl}`);
  }

  getById(id: number): Observable<FeeType> {
    return this.http.get<FeeType>(`${this.baseUrl}/${id}`);
  }

  create(dto: CreateFeeTypeDto): Observable<FeeType> {
    return this.http.post<FeeType>(`${this.baseUrl}`, dto);
  }

  update(id: number, dto: UpdateFeeTypeDto): Observable<FeeType> {
    return this.http.put<FeeType>(`${this.baseUrl}/${id}`, dto);
  }
