...
def generate_strategic_recommendations(prediction, input_data):
    ...
    # إنشاء توصيات بناءً على النتيجة والبيانات
    recommendations = {
        "student": {
            "title": "استراتيجية للطلاب",
            "actions": [
                "برامج تعليمية شخصية",
                "أنشطة تقوية أكاديمية",
                "مجموعات دراسية مشرفة",
        ...
        "teacher": {
            ...
            "actions": [
                "تدريب تعليمي مستمر",
                "ورش عمل حول طرق التدريس الجديدة",
...
def main():
    ...
    if models is None:
        ...
        st.info("لإنشاء النماذج، قم بتشغيل: python ml_core/model_trainer.py")
        return
...
def show_single_school_analysis(models):
    ...
    with st.form("school_data_form"):
        ...
        if submitted:
            ...
            if prediction:
                display_results(prediction, input_data, school_name, models)
