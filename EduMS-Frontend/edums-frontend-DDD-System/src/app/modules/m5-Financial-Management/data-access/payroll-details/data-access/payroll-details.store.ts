...
import { PayrollDetail, CreatePayrollDetailDto, UpdatePayrollDetailDto } from '@modules/m5-Financial-Management/interfaces/payroll-detail.interface';

type PayrollDetailsStoreState = {
  items: PayrollDetail[];
  selectedItem: PayrollDetail | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: PayrollDetailsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const PayrollDetailsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(PayrollDetailsService)) => ({
    ...
    create: rxMethod<CreatePayrollDetailDto>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as PayrollDetail] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdatePayrollDetailDto}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as PayrollDetail[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as PayrollDetail[], isLoading: false });
            },
