...
import { FeeStructure, CreateFeeStructurePayload, UpdateFeeStructurePayload } from '../../../../core/api/interfaces/M5_FinancialManagement/feestructure.interface';

type FeeStructuresStoreState = {
  items: FeeStructure[];
  selectedItem: FeeStructure | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: FeeStructuresStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const FeeStructuresStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(FeeStructuresService)) => ({
    ...
    create: rxMethod<CreateFeeStructurePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as FeeStructure] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateFeeStructurePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as FeeStructure[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as FeeStructure[], isLoading: false });
            },
