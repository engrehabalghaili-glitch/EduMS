...
import { JournalEntry, CreateJournalEntryPayload, UpdateJournalEntryPayload } from '../../../../core/api/interfaces/M5_FinancialManagement/journalentry.interface';

type JournalEntriesStoreState = {
  items: JournalEntry[];
...
const initialState: JournalEntriesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const JournalEntriesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(JournalEntriesService)) => ({
    ...
    create: rxMethod<CreateJournalEntryPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as JournalEntry] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateJournalEntryPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as JournalEntry[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as JournalEntry[], isLoading: false });
            },
