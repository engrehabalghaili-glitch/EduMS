...
import { StudentInvoice, CreateStudentInvoicePayload, UpdateStudentInvoicePayload } from '../../../../core/api/interfaces/M5_FinancialManagement/studentinvoice.interface';

type StudentInvoicesStoreState = {
  items: StudentInvoice[];
  selectedItem: StudentInvoice | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentInvoicesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentInvoicesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentInvoicesService)) => ({
    ...
    create: rxMethod<CreateStudentInvoicePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentInvoice] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateStudentInvoicePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentInvoice[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentInvoice[], isLoading: false });
            },
