...
import { InvoiceItem, CreateInvoiceItemPayload, UpdateInvoiceItemPayload } from '../../../../core/api/interfaces/M5_FinancialManagement/invoiceitem.interface';

type InvoiceItemsStoreState = {
  items: InvoiceItem[];
...
const initialState: InvoiceItemsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const InvoiceItemsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(InvoiceItemsService)) => ({
    ...
    create: rxMethod<CreateInvoiceItemPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as InvoiceItem] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateInvoiceItemPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as InvoiceItem[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as InvoiceItem[], isLoading: false });
            },
