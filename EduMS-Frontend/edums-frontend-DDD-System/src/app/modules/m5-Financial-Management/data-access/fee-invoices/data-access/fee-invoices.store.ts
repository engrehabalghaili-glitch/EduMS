...
import { FeeInvoice, CreateFeeInvoicePayload, UpdateFeeInvoicePayload } from '../../../../core/api/interfaces/M5_FinancialManagement/feeinvoice.interface';

type FeeInvoicesStoreState = {
  items: FeeInvoice[];
  selectedItem: FeeInvoice | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: FeeInvoicesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const FeeInvoicesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(FeeInvoicesService)) => ({
    ...
    create: rxMethod<CreateFeeInvoicePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as FeeInvoice] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateFeeInvoicePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as FeeInvoice[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as FeeInvoice[], isLoading: false });
            },
