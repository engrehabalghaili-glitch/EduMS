...
import { FeeType, CreateFeeTypePayload, UpdateFeeTypePayload } from '../../../../core/api/interfaces/M5_FinancialManagement/feetype.interface';

type FeeTypesStoreState = {
  items: FeeType[];
  selectedItem: FeeType | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: FeeTypesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const FeeTypesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(FeeTypesService)) => ({
    ...
    create: rxMethod<CreateFeeTypePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as FeeType] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateFeeTypePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as FeeType[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as FeeType[], isLoading: false });
            },
