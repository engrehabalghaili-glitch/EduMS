...
import { FeeInstallment, CreateFeeInstallmentDto, UpdateFeeInstallmentDto } from '@modules/m5-Financial-Management/interfaces/fee-installment.interface';
...
export const FeeInstallmentsStore = signalStore(
  ...
  withMethods((store, service = inject(FeeInstallmentsService)) => ({
    ...
    create: rxMethod<CreateFeeInstallmentDto>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
    ...
    update: rxMethod<{id: number | string, payload: UpdateFeeInstallmentDto}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
