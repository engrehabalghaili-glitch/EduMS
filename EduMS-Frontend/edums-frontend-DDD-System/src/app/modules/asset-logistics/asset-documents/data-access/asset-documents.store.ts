...
import { AssetDocument, CreateAssetDocumentPayload, UpdateAssetDocumentPayload } from '../../../../core/api/interfaces/M4_AssetLogistics/assetdocument.interface';

type AssetDocumentsStoreState = {
  items: AssetDocument[];
  selectedItem: AssetDocument | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetDocumentsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetDocumentsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetDocumentsService)) => ({
    ...
    create: rxMethod<CreateAssetDocumentPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetDocument] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateAssetDocumentPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetDocument[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetDocument[], isLoading: false });
            },
