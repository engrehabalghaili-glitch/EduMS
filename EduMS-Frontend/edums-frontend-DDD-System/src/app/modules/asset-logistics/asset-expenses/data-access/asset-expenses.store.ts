...
import { AssetExpensesService } from './asset-expenses.service';
import { AssetExpense, CreateAssetExpensePayload, UpdateAssetExpensePayload } from '../../../../core/api/interfaces/M4_AssetLogistics/assetexpense.interface';

type AssetExpensesStoreState = {
  items: AssetExpense[];
  selectedItem: AssetExpense | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetExpensesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetExpensesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetExpensesService)) => ({
    ...
    create: rxMethod<CreateAssetExpensePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetExpense] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateAssetExpensePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetExpense[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetExpense[], isLoading: false });
            },
