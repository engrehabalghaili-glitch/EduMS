...
type AssetFeasibilityRiskAnalysisesStoreState = {
  ...
  selectedItem: AssetFeasibilityRiskAnalysis | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetFeasibilityRiskAnalysisesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetFeasibilityRiskAnalysisesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetFeasibilityRiskAnalysisesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetFeasibilityRiskAnalysis] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetFeasibilityRiskAnalysis[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetFeasibilityRiskAnalysis[], isLoading: false });
            },
