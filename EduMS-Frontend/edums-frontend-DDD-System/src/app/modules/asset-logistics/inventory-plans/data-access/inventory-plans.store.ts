...
type InventoryPlansStoreState = {
  items: InventoryPlan[];
  selectedItem: InventoryPlan | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: InventoryPlansStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const InventoryPlansStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(InventoryPlansService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as InventoryPlan] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as InventoryPlan[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as InventoryPlan[], isLoading: false });
            },
