...
import { StudentHealthRecord, CreateStudentHealthRecordPayload, UpdateStudentHealthRecordPayload } from '../../../../core/api/interfaces/M2_StudentAffairs/studenthealthrecord.interface';

type StudentHealthRecordsStoreState = {
  items: StudentHealthRecord[];
  selectedItem: StudentHealthRecord | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentHealthRecordsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentHealthRecordsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentHealthRecordsService)) => ({
    ...
    create: rxMethod<CreateStudentHealthRecordPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentHealthRecord] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateStudentHealthRecordPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentHealthRecord[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentHealthRecord[], isLoading: false });
            },
