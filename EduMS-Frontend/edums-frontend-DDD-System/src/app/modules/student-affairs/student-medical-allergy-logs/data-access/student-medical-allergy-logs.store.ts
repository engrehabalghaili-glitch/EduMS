...
type StudentMedicalAllergyLogsStoreState = {
  ...
  selectedItem: StudentMedicalAllergyLog | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentMedicalAllergyLogsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentMedicalAllergyLogsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentMedicalAllergyLogsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentMedicalAllergyLog] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentMedicalAllergyLog[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentMedicalAllergyLog[], isLoading: false });
            },
