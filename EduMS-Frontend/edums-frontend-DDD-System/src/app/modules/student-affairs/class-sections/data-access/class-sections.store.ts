...
import { ClassSection, CreateClassSectionPayload, UpdateClassSectionPayload } from '../../../../core/api/interfaces/M2_StudentAffairs/classsection.interface';

type ClassSectionsStoreState = {
  items: ClassSection[];
  selectedItem: ClassSection | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: ClassSectionsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const ClassSectionsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(ClassSectionsService)) => ({
    ...
    create: rxMethod<CreateClassSectionPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as ClassSection] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateClassSectionPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ClassSection[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ClassSection[], isLoading: false });
            },
