...
type StudentTransportationSubscriptionsStoreState = {
  ...
  selectedItem: StudentTransportationSubscription | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentTransportationSubscriptionsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentTransportationSubscriptionsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentTransportationSubscriptionsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentTransportationSubscription] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentTransportationSubscription[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentTransportationSubscription[], isLoading: false });
            },
