...
import { StudentAssessment, CreateStudentAssessmentPayload, UpdateStudentAssessmentPayload } from '../../../../core/api/interfaces/M2_StudentAffairs/studentassessment.interface';

type StudentAssessmentsStoreState = {
  items: StudentAssessment[];
  selectedItem: StudentAssessment | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentAssessmentsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentAssessmentsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentAssessmentsService)) => ({
    ...
    create: rxMethod<CreateStudentAssessmentPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentAssessment] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateStudentAssessmentPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentAssessment[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentAssessment[], isLoading: false });
            },
