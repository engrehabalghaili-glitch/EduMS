...
import { EmployeeViolation, CreateEmployeeViolationPayload, UpdateEmployeeViolationPayload } from '../../../../core/api/interfaces/M3_EmployeeManagement/employeeviolation.interface';

type EmployeeViolationsStoreState = {
  items: EmployeeViolation[];
  selectedItem: EmployeeViolation | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmployeeViolationsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmployeeViolationsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmployeeViolationsService)) => ({
    ...
    create: rxMethod<CreateEmployeeViolationPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmployeeViolation] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateEmployeeViolationPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeViolation[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeViolation[], isLoading: false });
            },
