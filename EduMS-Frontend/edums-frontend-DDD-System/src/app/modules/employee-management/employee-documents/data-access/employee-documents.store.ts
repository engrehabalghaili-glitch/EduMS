...
import { EmployeeDocument, CreateEmployeeDocumentPayload, UpdateEmployeeDocumentPayload } from '../../../../core/api/interfaces/M3_EmployeeManagement/employeedocument.interface';

type EmployeeDocumentsStoreState = {
  items: EmployeeDocument[];
  selectedItem: EmployeeDocument | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmployeeDocumentsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmployeeDocumentsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmployeeDocumentsService)) => ({
    ...
    create: rxMethod<CreateEmployeeDocumentPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmployeeDocument] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateEmployeeDocumentPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeDocument[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeDocument[], isLoading: false });
            },
