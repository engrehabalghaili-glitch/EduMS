...
import { Employee, CreateEmployeePayload, UpdateEmployeePayload } from '../../../../core/api/interfaces/M3_EmployeeManagement/employee.interface';

type EmployeesStoreState = {
  items: Employee[];
...
export const EmployeesStore = signalStore(
  ...
  withMethods((store, service = inject(EmployeesService)) => ({
    ...
    create: rxMethod<CreateEmployeePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as Employee] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateEmployeePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Employee[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Employee[], isLoading: false });
            },
