...
type EmployeePerformanceReviewsStoreState = {
  ...
  selectedItem: EmployeePerformanceReview | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmployeePerformanceReviewsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmployeePerformanceReviewsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmployeePerformanceReviewsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmployeePerformanceReview] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeePerformanceReview[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeePerformanceReview[], isLoading: false });
            },
