...
import { EmployeeTrainingsService } from './employee-trainings.service';
import { EmployeeTraining, CreateEmployeeTrainingPayload, UpdateEmployeeTrainingPayload } from '../../../../core/api/interfaces/M3_EmployeeManagement/employeetraining.interface';

type EmployeeTrainingsStoreState = {
  items: EmployeeTraining[];
  selectedItem: EmployeeTraining | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmployeeTrainingsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmployeeTrainingsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmployeeTrainingsService)) => ({
    ...
    create: rxMethod<CreateEmployeeTrainingPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmployeeTraining] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateEmployeeTrainingPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeTraining[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeTraining[], isLoading: false });
            },
