...
import { SystemPermission, CreateSystemPermissionPayload, UpdateSystemPermissionPayload } from '../../../../core/api/interfaces/M8_AuthenticationUsers/systempermission.interface';

type SystemPermissionsStoreState = {
  items: SystemPermission[];
  selectedItem: SystemPermission | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SystemPermissionsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SystemPermissionsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SystemPermissionsService)) => ({
    ...
    create: rxMethod<CreateSystemPermissionPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SystemPermission] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateSystemPermissionPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SystemPermission[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SystemPermission[], isLoading: false });
            },
