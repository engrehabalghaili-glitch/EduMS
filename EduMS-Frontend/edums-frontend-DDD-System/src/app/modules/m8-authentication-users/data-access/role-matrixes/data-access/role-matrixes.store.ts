...
type RoleMatrixesStoreState = {
  items: RoleMatrix[];
  selectedItem: RoleMatrix | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: RoleMatrixesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const RoleMatrixesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(RoleMatrixesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as RoleMatrix] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as RoleMatrix[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as RoleMatrix[], isLoading: false });
            },
