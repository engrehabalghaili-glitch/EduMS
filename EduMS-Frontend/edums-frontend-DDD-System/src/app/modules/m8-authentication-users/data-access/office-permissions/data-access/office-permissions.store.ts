...
type OfficePermissionsStoreState = {
  ...
  selectedItem: OfficePermission | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: OfficePermissionsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const OfficePermissionsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(OfficePermissionsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as OfficePermission] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as OfficePermission[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as OfficePermission[], isLoading: false });
            },
