...
import { UserActivityLog, CreateUserActivityLogPayload, UpdateUserActivityLogPayload } from '../../../../core/api/interfaces/M8_AuthenticationUsers/useractivitylog.interface';

type UserActivityLogsStoreState = {
  items: UserActivityLog[];
  selectedItem: UserActivityLog | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: UserActivityLogsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const UserActivityLogsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(UserActivityLogsService)) => ({
    ...
    create: rxMethod<CreateUserActivityLogPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as UserActivityLog] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateUserActivityLogPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as UserActivityLog[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as UserActivityLog[], isLoading: false });
            },
