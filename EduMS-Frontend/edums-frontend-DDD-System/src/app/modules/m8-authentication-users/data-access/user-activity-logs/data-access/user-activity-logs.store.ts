...
import { UserActivityLog, CreateUserActivityLog, UpdateUserActivityLog } from '@modules/m8-authentication-users/interfaces/user-activity-log.models';
...
export const UserActivityLogsStore = signalStore(
  ...
  withMethods((store, service = inject(UserActivityLogsService)) => ({
    ...
    create: rxMethod<CreateUserActivityLog>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
    ...
    update: rxMethod<{id: number | string, payload: UpdateUserActivityLog}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
