...
type PrivilegeRulesStoreState = {
  items: PrivilegeRule[];
  selectedItem: PrivilegeRule | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: PrivilegeRulesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const PrivilegeRulesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(PrivilegeRulesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as PrivilegeRule] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as PrivilegeRule[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as PrivilegeRule[], isLoading: false });
            },
