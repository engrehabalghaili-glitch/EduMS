...
type RolePermissionsStoreState = {
  ...
  selectedItem: RolePermission | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: RolePermissionsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const RolePermissionsStore = signalStore(
  ...
  withMethods((store, service = inject(RolePermissionsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as RolePermission] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as RolePermission[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as RolePermission[], isLoading: false });
            },
