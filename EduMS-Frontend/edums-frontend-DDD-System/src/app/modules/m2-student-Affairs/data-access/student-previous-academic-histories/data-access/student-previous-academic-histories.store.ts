...
type StudentPreviousAcademicHistoriesStoreState = {
  ...
  selectedItem: StudentPreviousAcademicHistory | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentPreviousAcademicHistoriesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentPreviousAcademicHistoriesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentPreviousAcademicHistoriesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentPreviousAcademicHistory] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentPreviousAcademicHistory[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentPreviousAcademicHistory[], isLoading: false });
            },
