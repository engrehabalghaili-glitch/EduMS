...
import { StudentEnrollment, CreateStudentEnrollmentPayload, UpdateStudentEnrollmentPayload } from '../../../../core/api/interfaces/M2_StudentAffairs/studentenrollment.interface';

type StudentEnrollmentsStoreState = {
  items: StudentEnrollment[];
  selectedItem: StudentEnrollment | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentEnrollmentsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentEnrollmentsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentEnrollmentsService)) => ({
    ...
    create: rxMethod<CreateStudentEnrollmentPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentEnrollment] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateStudentEnrollmentPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentEnrollment[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentEnrollment[], isLoading: false });
            },
