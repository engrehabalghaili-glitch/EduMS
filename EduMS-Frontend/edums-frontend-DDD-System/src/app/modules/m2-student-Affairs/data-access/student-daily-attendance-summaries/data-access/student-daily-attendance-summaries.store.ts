...
type StudentDailyAttendanceSummariesStoreState = {
  ...
  selectedItem: StudentDailyAttendanceSummary | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentDailyAttendanceSummariesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentDailyAttendanceSummariesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentDailyAttendanceSummariesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentDailyAttendanceSummary] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentDailyAttendanceSummary[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentDailyAttendanceSummary[], isLoading: false });
            },
