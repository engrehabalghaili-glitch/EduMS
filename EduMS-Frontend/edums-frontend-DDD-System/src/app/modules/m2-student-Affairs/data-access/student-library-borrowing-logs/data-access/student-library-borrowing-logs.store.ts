...
type StudentLibraryBorrowingLogsStoreState = {
  ...
  selectedItem: StudentLibraryBorrowingLog | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentLibraryBorrowingLogsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentLibraryBorrowingLogsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentLibraryBorrowingLogsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentLibraryBorrowingLog] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentLibraryBorrowingLog[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentLibraryBorrowingLog[], isLoading: false });
            },
