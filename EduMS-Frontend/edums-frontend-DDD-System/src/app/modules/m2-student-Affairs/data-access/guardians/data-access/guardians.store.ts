...
import { Guardian, CreateGuardianPayload, UpdateGuardianPayload } from '../../../../core/api/interfaces/M2_StudentAffairs/guardian.interface';

type GuardiansStoreState = {
  items: Guardian[];
  selectedItem: Guardian | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: GuardiansStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const GuardiansStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(GuardiansService)) => ({
    ...
    create: rxMethod<CreateGuardianPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as Guardian] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateGuardianPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Guardian[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Guardian[], isLoading: false });
            },
