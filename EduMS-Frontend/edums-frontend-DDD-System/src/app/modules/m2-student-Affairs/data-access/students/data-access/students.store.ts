...
import { Student, CreateStudentPayload, UpdateStudentPayload } from '../../../../core/api/interfaces/M2_StudentAffairs/student.interface';
...
export const StudentsStore = signalStore(
  ...
  withMethods((store, service = inject(StudentsService)) => ({
    ...
    create: rxMethod<CreateStudentPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as Student] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateStudentPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Student[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Student[], isLoading: false });
            },
