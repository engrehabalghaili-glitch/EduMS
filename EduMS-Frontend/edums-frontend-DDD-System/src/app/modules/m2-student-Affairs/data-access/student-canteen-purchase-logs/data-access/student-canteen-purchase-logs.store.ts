...
type StudentCanteenPurchaseLogsStoreState = {
  ...
  selectedItem: StudentCanteenPurchaseLog | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentCanteenPurchaseLogsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentCanteenPurchaseLogsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentCanteenPurchaseLogsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentCanteenPurchaseLog] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentCanteenPurchaseLog[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentCanteenPurchaseLog[], isLoading: false });
            },
