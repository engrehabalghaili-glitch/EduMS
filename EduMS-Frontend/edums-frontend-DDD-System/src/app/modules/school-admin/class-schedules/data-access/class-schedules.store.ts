...
import { ClassSchedule, CreateClassSchedulePayload, UpdateClassSchedulePayload } from '../../../../core/api/interfaces/M1_SchoolAdmin/classschedule.interface';

type ClassSchedulesStoreState = {
  items: ClassSchedule[];
  selectedItem: ClassSchedule | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: ClassSchedulesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const ClassSchedulesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(ClassSchedulesService)) => ({
    ...
    create: rxMethod<CreateClassSchedulePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as ClassSchedule] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateClassSchedulePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ClassSchedule[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ClassSchedule[], isLoading: false });
            },
