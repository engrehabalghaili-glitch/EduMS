...
import { EducationalStage, CreateEducationalStagePayload, UpdateEducationalStagePayload } from '../../../../core/api/interfaces/M1_SchoolAdmin/educationalstage.interface';

type EducationalStagesStoreState = {
  items: EducationalStage[];
  selectedItem: EducationalStage | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EducationalStagesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EducationalStagesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EducationalStagesService)) => ({
    ...
    create: rxMethod<CreateEducationalStagePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EducationalStage] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateEducationalStagePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EducationalStage[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EducationalStage[], isLoading: false });
            },
