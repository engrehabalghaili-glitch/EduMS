...
type GradingScaleBoundsStoreState = {
  ...
  selectedItem: GradingScaleBound | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: GradingScaleBoundsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const GradingScaleBoundsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(GradingScaleBoundsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as GradingScaleBound] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as GradingScaleBound[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as GradingScaleBound[], isLoading: false });
            },
