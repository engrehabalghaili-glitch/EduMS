...
type ClassroomResourceAllocationsStoreState = {
  ...
  selectedItem: ClassroomResourceAllocation | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: ClassroomResourceAllocationsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const ClassroomResourceAllocationsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(ClassroomResourceAllocationsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as ClassroomResourceAllocation] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ClassroomResourceAllocation[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ClassroomResourceAllocation[], isLoading: false });
            },
