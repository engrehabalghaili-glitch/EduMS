...
type ReferenceCodingLookupsStoreState = {
  ...
  selectedItem: ReferenceCodingLookup | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: ReferenceCodingLookupsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const ReferenceCodingLookupsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(ReferenceCodingLookupsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as ReferenceCodingLookup] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ReferenceCodingLookup[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ReferenceCodingLookup[], isLoading: false });
            },
