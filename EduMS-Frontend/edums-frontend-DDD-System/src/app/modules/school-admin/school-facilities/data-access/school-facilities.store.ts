...
import { SchoolFacilitiesService } from './school-facilities.service';
import { SchoolFacility, CreateSchoolFacilityPayload, UpdateSchoolFacilityPayload } from '../../../../core/api/interfaces/M1_SchoolAdmin/schoolfacility.interface';

type SchoolFacilitiesStoreState = {
  items: SchoolFacility[];
  selectedItem: SchoolFacility | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SchoolFacilitiesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SchoolFacilitiesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SchoolFacilitiesService)) => ({
    ...
    create: rxMethod<CreateSchoolFacilityPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SchoolFacility] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateSchoolFacilityPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolFacility[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolFacility[], isLoading: false });
            },
