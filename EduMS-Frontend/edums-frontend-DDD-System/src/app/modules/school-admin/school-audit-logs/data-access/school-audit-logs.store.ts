...
import { SchoolAuditLogsService } from './school-audit-logs.service';
import { SchoolAuditLog, CreateSchoolAuditLogPayload, UpdateSchoolAuditLogPayload } from '../../../../core/api/interfaces/M1_SchoolAdmin/schoolauditlog.interface';

type SchoolAuditLogsStoreState = {
  items: SchoolAuditLog[];
  selectedItem: SchoolAuditLog | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SchoolAuditLogsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SchoolAuditLogsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SchoolAuditLogsService)) => ({
    ...
    create: rxMethod<CreateSchoolAuditLogPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SchoolAuditLog] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateSchoolAuditLogPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolAuditLog[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolAuditLog[], isLoading: false });
            },
