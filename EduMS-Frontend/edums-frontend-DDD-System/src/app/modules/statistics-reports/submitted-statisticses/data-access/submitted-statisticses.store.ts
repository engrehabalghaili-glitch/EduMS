...
type SubmittedStatisticsesStoreState = {
  ...
  selectedItem: SubmittedStatistics | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SubmittedStatisticsesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SubmittedStatisticsesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SubmittedStatisticsesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SubmittedStatistics] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SubmittedStatistics[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SubmittedStatistics[], isLoading: false });
            },
