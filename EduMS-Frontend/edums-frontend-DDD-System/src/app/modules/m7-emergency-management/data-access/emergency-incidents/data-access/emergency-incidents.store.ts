...
import { EmergencyIncident, CreateEmergencyIncidentPayload, UpdateEmergencyIncidentPayload } from '../../../../core/api/interfaces/M7_EmergencyManagement/emergencyincident.interface';

type EmergencyIncidentsStoreState = {
  items: EmergencyIncident[];
  selectedItem: EmergencyIncident | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmergencyIncidentsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmergencyIncidentsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmergencyIncidentsService)) => ({
    ...
    create: rxMethod<CreateEmergencyIncidentPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmergencyIncident] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateEmergencyIncidentPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmergencyIncident[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmergencyIncident[], isLoading: false });
            },
