...
import { EmergencyClosure, CreateEmergencyClosure, UpdateEmergencyClosure } from '@modules/m7-emergency-management/interfaces/emergency-closure.types';
...
export const EmergencyClosuresStore = signalStore(
  ...
  withMethods((store, service = inject(EmergencyClosuresService)) => ({
    ...
    create: rxMethod<CreateEmergencyClosure>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
    ...
    update: rxMethod<{id: number | string, payload: UpdateEmergencyClosure}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
