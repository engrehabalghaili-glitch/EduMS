...
type EmergencyPlansStoreState = {
  items: EmergencyPlan[];
  selectedItem: EmergencyPlan | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmergencyPlansStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmergencyPlansStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmergencyPlansService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmergencyPlan] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmergencyPlan[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmergencyPlan[], isLoading: false });
            },
