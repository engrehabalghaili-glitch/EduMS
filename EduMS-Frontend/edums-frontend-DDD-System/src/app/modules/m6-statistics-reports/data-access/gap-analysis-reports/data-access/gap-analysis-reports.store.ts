...
type GapAnalysisReportsStoreState = {
  ...
  selectedItem: GapAnalysisReport | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: GapAnalysisReportsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const GapAnalysisReportsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(GapAnalysisReportsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as GapAnalysisReport] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as GapAnalysisReport[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as GapAnalysisReport[], isLoading: false });
            },
