...
type KpiMetricRecordsStoreState = {
  ...
  selectedItem: KpiMetricRecord | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: KpiMetricRecordsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const KpiMetricRecordsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(KpiMetricRecordsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as KpiMetricRecord] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as KpiMetricRecord[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as KpiMetricRecord[], isLoading: false });
            },
