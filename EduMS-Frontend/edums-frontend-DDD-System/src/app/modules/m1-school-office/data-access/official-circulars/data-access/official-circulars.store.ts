...
type OfficialCircularsStoreState = {
  ...
  selectedItem: OfficialCircular | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: OfficialCircularsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const OfficialCircularsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(OfficialCircularsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as OfficialCircular] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as OfficialCircular[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as OfficialCircular[], isLoading: false });
            },
