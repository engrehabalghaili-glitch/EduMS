...
type CurriculumTextbookDistributionsStoreState = {
  ...
  selectedItem: CurriculumTextbookDistribution | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: CurriculumTextbookDistributionsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const CurriculumTextbookDistributionsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(CurriculumTextbookDistributionsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as CurriculumTextbookDistribution] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as CurriculumTextbookDistribution[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as CurriculumTextbookDistribution[], isLoading: false });
            },
