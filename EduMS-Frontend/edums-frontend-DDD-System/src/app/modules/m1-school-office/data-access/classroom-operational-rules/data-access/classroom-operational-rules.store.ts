...
type ClassroomOperationalRulesStoreState = {
  ...
  selectedItem: ClassroomOperationalRule | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: ClassroomOperationalRulesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const ClassroomOperationalRulesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(ClassroomOperationalRulesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as ClassroomOperationalRule] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ClassroomOperationalRule[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ClassroomOperationalRule[], isLoading: false });
            },
