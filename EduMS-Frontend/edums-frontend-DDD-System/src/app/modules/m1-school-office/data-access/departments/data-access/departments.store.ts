...
import { Department, CreateDepartmentPayload, UpdateDepartmentPayload } from '../../../../core/api/interfaces/M1_SchoolAdmin/department.interface';

type DepartmentsStoreState = {
  items: Department[];
...
export const DepartmentsStore = signalStore(
  ...
  withMethods((store, service = inject(DepartmentsService)) => ({
    ...
    create: rxMethod<CreateDepartmentPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as Department] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateDepartmentPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Department[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Department[], isLoading: false });
            },
