...
import { SchoolSemestersService } from './school-semesters.service';
import { SchoolSemester, CreateSchoolSemesterDto, UpdateSchoolSemesterDto } from '@modules/m1-school-office/interface/school-semester';

type SchoolSemestersStoreState = {
  items: SchoolSemester[];
  selectedItem: SchoolSemester | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SchoolSemestersStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SchoolSemestersStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SchoolSemestersService)) => ({
    ...
    create: rxMethod<CreateSchoolSemesterDto>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SchoolSemester] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateSchoolSemesterDto}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolSemester[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolSemester[], isLoading: false });
            },
