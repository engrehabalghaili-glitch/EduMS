...
type DirectorateExamCenterAssignmentsStoreState = {
  ...
  selectedItem: DirectorateExamCenterAssignment | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: DirectorateExamCenterAssignmentsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const DirectorateExamCenterAssignmentsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(DirectorateExamCenterAssignmentsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as DirectorateExamCenterAssignment] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as DirectorateExamCenterAssignment[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as DirectorateExamCenterAssignment[], isLoading: false });
            },
