...
import { SchoolLevel, CreateSchoolLevelPayload, UpdateSchoolLevelPayload } from '../../../../core/api/interfaces/M1_SchoolAdmin/schoollevel.interface';

type SchoolLevelsStoreState = {
  items: SchoolLevel[];
  selectedItem: SchoolLevel | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SchoolLevelsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SchoolLevelsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SchoolLevelsService)) => ({
    ...
    create: rxMethod<CreateSchoolLevelPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SchoolLevel] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateSchoolLevelPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolLevel[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolLevel[], isLoading: false });
            },
