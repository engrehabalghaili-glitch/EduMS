...
type GradeCapacitiesStoreState = {
  items: GradeCapacity[];
  selectedItem: GradeCapacity | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: GradeCapacitiesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const GradeCapacitiesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(GradeCapacitiesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as GradeCapacity] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as GradeCapacity[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as GradeCapacity[], isLoading: false });
            },
