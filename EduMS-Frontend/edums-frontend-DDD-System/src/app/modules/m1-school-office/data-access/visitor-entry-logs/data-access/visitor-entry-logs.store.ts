...
type VisitorEntryLogsStoreState = {
  ...
  selectedItem: VisitorEntryLog | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: VisitorEntryLogsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const VisitorEntryLogsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(VisitorEntryLogsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as VisitorEntryLog] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as VisitorEntryLog[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as VisitorEntryLog[], isLoading: false });
            },
