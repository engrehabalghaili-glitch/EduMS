...
type DirectorateLegalCaseLogsStoreState = {
  ...
  selectedItem: DirectorateLegalCaseLog | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: DirectorateLegalCaseLogsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const DirectorateLegalCaseLogsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(DirectorateLegalCaseLogsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as DirectorateLegalCaseLog] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as DirectorateLegalCaseLog[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as DirectorateLegalCaseLog[], isLoading: false });
            },
