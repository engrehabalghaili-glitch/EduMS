...
import { Classroom, CreateClassroomDto, UpdateClassroomDto } from '@modules/m1-school-office/interface/classroom';

type ClassroomsStoreState = {
  items: Classroom[];
  selectedItem: Classroom | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: ClassroomsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const ClassroomsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(ClassroomsService)) => ({
    ...
    create: rxMethod<CreateClassroomDto>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as Classroom] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateClassroomDto}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Classroom[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Classroom[], isLoading: false });
            },
