...
import { SchoolShiftsService } from './school-shifts.service';
import { SchoolShift, CreateSchoolShiftPayload, UpdateSchoolShiftPayload } from '../../../../core/api/interfaces/M1_SchoolAdmin/schoolshift.interface';

type SchoolShiftsStoreState = {
  items: SchoolShift[];
  selectedItem: SchoolShift | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SchoolShiftsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SchoolShiftsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SchoolShiftsService)) => ({
    ...
    create: rxMethod<CreateSchoolShiftPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SchoolShift] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateSchoolShiftPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolShift[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolShift[], isLoading: false });
            },
