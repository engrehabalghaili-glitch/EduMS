...
type BehaviorPermissionMatrixesStoreState = {
  ...
  selectedItem: BehaviorPermissionMatrix | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: BehaviorPermissionMatrixesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const BehaviorPermissionMatrixesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(BehaviorPermissionMatrixesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as BehaviorPermissionMatrix] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as BehaviorPermissionMatrix[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as BehaviorPermissionMatrix[], isLoading: false });
            },
