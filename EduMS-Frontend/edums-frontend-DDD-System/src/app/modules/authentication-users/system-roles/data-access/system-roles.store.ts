...
import { SystemRole, CreateSystemRolePayload, UpdateSystemRolePayload } from '../../../../core/api/interfaces/M8_AuthenticationUsers/systemrole.interface';

type SystemRolesStoreState = {
  items: SystemRole[];
  selectedItem: SystemRole | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SystemRolesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SystemRolesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SystemRolesService)) => ({
    ...
    create: rxMethod<CreateSystemRolePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SystemRole] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateSystemRolePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SystemRole[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SystemRole[], isLoading: false });
            },
