...
import { AccessPolicy, CreateAccessPolicyPayload, UpdateAccessPolicyPayload } from '../../../../core/api/interfaces/M8_AuthenticationUsers/accesspolicy.interface';

type AccessPoliciesStoreState = {
  items: AccessPolicy[];
  selectedItem: AccessPolicy | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AccessPoliciesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AccessPoliciesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AccessPoliciesService)) => ({
    ...
    create: rxMethod<CreateAccessPolicyPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AccessPolicy] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateAccessPolicyPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AccessPolicy[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AccessPolicy[], isLoading: false });
            },
