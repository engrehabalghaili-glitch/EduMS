...
type BehaviorPermissionsStoreState = {
  ...
  selectedItem: BehaviorPermission | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: BehaviorPermissionsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const BehaviorPermissionsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(BehaviorPermissionsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as BehaviorPermission] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as BehaviorPermission[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as BehaviorPermission[], isLoading: false });
            },
