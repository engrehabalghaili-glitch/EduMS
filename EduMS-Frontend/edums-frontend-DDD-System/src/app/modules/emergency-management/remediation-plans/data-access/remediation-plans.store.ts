...
import { RemediationPlansService } from './remediation-plans.service';
import { RemediationPlan, CreateRemediationPlanPayload, UpdateRemediationPlanPayload } from '../../../../core/api/interfaces/M7_EmergencyManagement/remediationplan.interface';

type RemediationPlansStoreState = {
  items: RemediationPlan[];
  selectedItem: RemediationPlan | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: RemediationPlansStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const RemediationPlansStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(RemediationPlansService)) => ({
    ...
    create: rxMethod<CreateRemediationPlanPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as RemediationPlan] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateRemediationPlanPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as RemediationPlan[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as RemediationPlan[], isLoading: false });
            },
