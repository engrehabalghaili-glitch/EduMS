...
type EmergencyHostingsStoreState = {
  ...
  selectedItem: EmergencyHosting | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmergencyHostingsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmergencyHostingsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmergencyHostingsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmergencyHosting] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmergencyHosting[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmergencyHosting[], isLoading: false });
            },
