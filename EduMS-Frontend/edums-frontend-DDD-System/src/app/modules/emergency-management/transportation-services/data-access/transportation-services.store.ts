...
import { TransportationService, CreateTransportationServicePayload, UpdateTransportationServicePayload } from '../../../../core/api/interfaces/M7_EmergencyManagement/transportationservice.interface';

type TransportationServicesStoreState = {
  items: TransportationService[];
  selectedItem: TransportationService | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: TransportationServicesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const TransportationServicesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(TransportationServicesService)) => ({
    ...
    create: rxMethod<CreateTransportationServicePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as TransportationService] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateTransportationServicePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as TransportationService[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as TransportationService[], isLoading: false });
            },
