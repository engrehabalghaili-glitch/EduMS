...
type SafetySecurityReportsStoreState = {
  ...
  selectedItem: SafetySecurityReport | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SafetySecurityReportsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SafetySecurityReportsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SafetySecurityReportsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SafetySecurityReport] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SafetySecurityReport[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SafetySecurityReport[], isLoading: false });
            },
