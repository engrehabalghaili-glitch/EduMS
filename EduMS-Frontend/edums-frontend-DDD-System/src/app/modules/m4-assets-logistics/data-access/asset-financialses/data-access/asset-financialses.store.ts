...
type AssetFinancialsesStoreState = {
  ...
  selectedItem: AssetFinancials | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetFinancialsesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetFinancialsesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetFinancialsesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetFinancials] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetFinancials[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetFinancials[], isLoading: false });
            },
