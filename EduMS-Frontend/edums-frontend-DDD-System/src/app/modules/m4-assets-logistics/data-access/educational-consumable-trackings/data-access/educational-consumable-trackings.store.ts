...
type EducationalConsumableTrackingsStoreState = {
  ...
  selectedItem: EducationalConsumableTracking | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EducationalConsumableTrackingsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EducationalConsumableTrackingsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EducationalConsumableTrackingsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EducationalConsumableTracking] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EducationalConsumableTracking[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EducationalConsumableTracking[], isLoading: false });
            },
