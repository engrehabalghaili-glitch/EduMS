...
import { AssetUsageLogsService } from './asset-usage-logs.service';
import { AssetUsageLog, CreateAssetUsageLogPayload, UpdateAssetUsageLogPayload } from '../../../../core/api/interfaces/M4_AssetLogistics/assetusagelog.interface';

type AssetUsageLogsStoreState = {
  items: AssetUsageLog[];
  selectedItem: AssetUsageLog | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetUsageLogsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetUsageLogsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetUsageLogsService)) => ({
    ...
    create: rxMethod<CreateAssetUsageLogPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetUsageLog] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateAssetUsageLogPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetUsageLog[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetUsageLog[], isLoading: false });
            },
