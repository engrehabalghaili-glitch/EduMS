...
type AssetMaintenanceTicketsStoreState = {
  ...
  selectedItem: AssetMaintenanceTicket | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetMaintenanceTicketsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetMaintenanceTicketsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetMaintenanceTicketsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetMaintenanceTicket] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetMaintenanceTicket[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetMaintenanceTicket[], isLoading: false });
            },
