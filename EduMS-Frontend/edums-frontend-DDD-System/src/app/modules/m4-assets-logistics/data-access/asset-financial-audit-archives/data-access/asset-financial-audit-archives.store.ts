...
type AssetFinancialAuditArchivesStoreState = {
  ...
  selectedItem: AssetFinancialAuditArchive | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetFinancialAuditArchivesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetFinancialAuditArchivesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetFinancialAuditArchivesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetFinancialAuditArchive] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetFinancialAuditArchive[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetFinancialAuditArchive[], isLoading: false });
            },
