...
type InventoryReconciliationsStoreState = {
  ...
  selectedItem: InventoryReconciliation | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: InventoryReconciliationsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const InventoryReconciliationsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(InventoryReconciliationsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as InventoryReconciliation] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as InventoryReconciliation[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as InventoryReconciliation[], isLoading: false });
            },
