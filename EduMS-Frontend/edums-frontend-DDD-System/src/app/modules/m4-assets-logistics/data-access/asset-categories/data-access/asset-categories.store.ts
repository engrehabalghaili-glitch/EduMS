...
import { AssetCategory, CreateAssetCategoryRequest, UpdateAssetCategoryRequest } from '@modules/m4-assets-logistics/interfaces/asset-categories';

type AssetCategoriesStoreState = {
  items: AssetCategory[];
  selectedItem: AssetCategory | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetCategoriesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetCategoriesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetCategoriesService)) => ({
    ...
    create: rxMethod<CreateAssetCategoryRequest>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetCategory] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateAssetCategoryRequest}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetCategory[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetCategory[], isLoading: false });
            },
