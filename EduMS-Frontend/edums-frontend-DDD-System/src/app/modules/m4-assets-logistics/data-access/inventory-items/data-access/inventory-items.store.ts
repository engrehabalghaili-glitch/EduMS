...
import { InventoryItem, CreateInventoryItemPayload, UpdateInventoryItemPayload } from '../../../../core/api/interfaces/M4_AssetLogistics/inventoryitem.interface';

type InventoryItemsStoreState = {
  items: InventoryItem[];
...
const initialState: InventoryItemsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const InventoryItemsStore = signalStore(
  ...
  withMethods((store, service = inject(InventoryItemsService)) => ({
    ...
    create: rxMethod<CreateInventoryItemPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as InventoryItem] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateInventoryItemPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as InventoryItem[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as InventoryItem[], isLoading: false });
            },
