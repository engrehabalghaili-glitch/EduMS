...
type MaintenanceSparePartsStoreState = {
  ...
  selectedItem: MaintenanceSparePart | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: MaintenanceSparePartsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const MaintenanceSparePartsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(MaintenanceSparePartsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as MaintenanceSparePart] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as MaintenanceSparePart[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as MaintenanceSparePart[], isLoading: false });
            },
