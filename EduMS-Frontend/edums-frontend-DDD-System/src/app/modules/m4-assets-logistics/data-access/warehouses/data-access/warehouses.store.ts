...
import { Warehouse, CreateWarehousePayload, UpdateWarehousePayload } from '../../../../core/api/interfaces/M4_AssetLogistics/warehouse.interface';

type WarehousesStoreState = {
  items: Warehouse[];
...
const initialState: WarehousesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const WarehousesStore = signalStore(
  ...
  withMethods((store, service = inject(WarehousesService)) => ({
    ...
    create: rxMethod<CreateWarehousePayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as Warehouse] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateWarehousePayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Warehouse[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Warehouse[], isLoading: false });
            },
