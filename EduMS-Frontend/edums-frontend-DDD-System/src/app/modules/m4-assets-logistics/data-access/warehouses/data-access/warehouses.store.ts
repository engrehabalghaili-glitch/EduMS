...
import { Warehouse, CreateWarehouseRequest, UpdateWarehouseRequest } from '@modules/m4-assets-logistics/interfaces/warehouses';
...
export const WarehousesStore = signalStore(
  ...
  withMethods((store, service = inject(WarehousesService)) => ({
    ...
    create: rxMethod<CreateWarehouseRequest>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
    ...
    update: rxMethod<{id: number | string, payload: UpdateWarehouseRequest}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
