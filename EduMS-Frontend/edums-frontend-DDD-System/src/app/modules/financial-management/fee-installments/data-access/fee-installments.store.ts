...
import { FeeInstallmentsService } from './fee-installments.service';
import { FeeInstallment, CreateFeeInstallmentPayload, UpdateFeeInstallmentPayload } from '../../../../core/api/interfaces/M5_FinancialManagement/feeinstallment.interface';

type FeeInstallmentsStoreState = {
  items: FeeInstallment[];
  selectedItem: FeeInstallment | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: FeeInstallmentsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const FeeInstallmentsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(FeeInstallmentsService)) => ({
    ...
    create: rxMethod<CreateFeeInstallmentPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as FeeInstallment] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateFeeInstallmentPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as FeeInstallment[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as FeeInstallment[], isLoading: false });
            },
