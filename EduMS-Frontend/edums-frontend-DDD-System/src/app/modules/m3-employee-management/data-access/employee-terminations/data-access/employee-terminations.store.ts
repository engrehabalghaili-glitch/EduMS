...
import { EmployeeTermination, CreateEmployeeTerminationPayload, UpdateEmployeeTerminationPayload } from '../../../../core/api/interfaces/M3_EmployeeManagement/employeetermination.interface';

type EmployeeTerminationsStoreState = {
  items: EmployeeTermination[];
  selectedItem: EmployeeTermination | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmployeeTerminationsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmployeeTerminationsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmployeeTerminationsService)) => ({
    ...
    create: rxMethod<CreateEmployeeTerminationPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmployeeTermination] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateEmployeeTerminationPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeTermination[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeTermination[], isLoading: false });
            },
