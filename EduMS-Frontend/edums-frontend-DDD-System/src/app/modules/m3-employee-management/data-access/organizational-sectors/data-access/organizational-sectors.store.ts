...
type OrganizationalSectorsStoreState = {
  ...
  selectedItem: OrganizationalSector | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: OrganizationalSectorsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const OrganizationalSectorsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(OrganizationalSectorsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as OrganizationalSector] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as OrganizationalSector[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as OrganizationalSector[], isLoading: false });
            },
