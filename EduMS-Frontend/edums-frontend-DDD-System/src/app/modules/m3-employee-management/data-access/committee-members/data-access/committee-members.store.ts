...
type CommitteeMembersStoreState = {
  ...
  selectedItem: CommitteeMember | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: CommitteeMembersStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const CommitteeMembersStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(CommitteeMembersService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as CommitteeMember] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as CommitteeMember[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as CommitteeMember[], isLoading: false });
            },
