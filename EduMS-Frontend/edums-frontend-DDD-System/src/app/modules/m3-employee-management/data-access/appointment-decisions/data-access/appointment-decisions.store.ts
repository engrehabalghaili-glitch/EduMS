...
type AppointmentDecisionsStoreState = {
  ...
  selectedItem: AppointmentDecision | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AppointmentDecisionsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AppointmentDecisionsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AppointmentDecisionsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AppointmentDecision] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AppointmentDecision[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AppointmentDecision[], isLoading: false });
            },
