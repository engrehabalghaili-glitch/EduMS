...
import { EmployeeMentorsService } from './employee-mentors.service';
import { EmployeeMentor, CreateEmployeeMentorPayload, UpdateEmployeeMentorPayload } from '../../../../core/api/interfaces/M3_EmployeeManagement/employeementor.interface';

type EmployeeMentorsStoreState = {
  items: EmployeeMentor[];
  selectedItem: EmployeeMentor | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmployeeMentorsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmployeeMentorsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmployeeMentorsService)) => ({
    ...
    create: rxMethod<CreateEmployeeMentorPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmployeeMentor] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateEmployeeMentorPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeMentor[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeeMentor[], isLoading: false });
            },
