...
type MeetingAttendanceRecordsStoreState = {
  ...
  selectedItem: MeetingAttendanceRecord | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: MeetingAttendanceRecordsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const MeetingAttendanceRecordsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(MeetingAttendanceRecordsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as MeetingAttendanceRecord] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as MeetingAttendanceRecord[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as MeetingAttendanceRecord[], isLoading: false });
            },
