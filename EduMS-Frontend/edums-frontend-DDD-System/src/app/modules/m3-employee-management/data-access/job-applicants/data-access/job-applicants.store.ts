...
import { JobApplicant, CreateJobApplicantPayload, UpdateJobApplicantPayload } from '../../../../core/api/interfaces/M3_EmployeeManagement/jobapplicant.interface';

type JobApplicantsStoreState = {
  items: JobApplicant[];
  selectedItem: JobApplicant | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: JobApplicantsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const JobApplicantsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(JobApplicantsService)) => ({
    ...
    create: rxMethod<CreateJobApplicantPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as JobApplicant] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateJobApplicantPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as JobApplicant[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as JobApplicant[], isLoading: false });
            },
