...
import { VacantPositionsService } from './vacant-positions.service';
import { VacantPosition, CreateVacantPositionPayload, UpdateVacantPositionPayload } from '../../../../core/api/interfaces/M3_EmployeeManagement/vacantposition.interface';

type VacantPositionsStoreState = {
  items: VacantPosition[];
  selectedItem: VacantPosition | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: VacantPositionsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const VacantPositionsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(VacantPositionsService)) => ({
    ...
    create: rxMethod<CreateVacantPositionPayload>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as VacantPosition] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateVacantPositionPayload}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as VacantPosition[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as VacantPosition[], isLoading: false });
            },
