...
import { EmployeePayroll, CreateEmployeePayroll, UpdateEmployeePayroll } from '@modules/m3-employee-management/interfaces/employee-payroll.types';
...
export const EmployeePayrollsStore = signalStore(
  ...
  withMethods((store, service = inject(EmployeePayrollsService)) => ({
    ...
    create: rxMethod<CreateEmployeePayroll>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
    ...
    update: rxMethod<{id: number | string, payload: UpdateEmployeePayroll}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
