"""
Create directory and file using basic commands
"""

import subprocess
import os

print("🔧 Creating data directory and file using system commands...")

# Create directory using mkdir (cross-platform)
try:
    if not os.path.exists('data'):
        os.makedirs('data')
        print("✓ Created data directory using os.makedirs")
    else:
        print("✓ Data directory already exists")
except Exception as e:
    print(f"✗ Error with os.makedirs: {e}")
    # Try subprocess as fallback
    try:
        subprocess.run(['mkdir', 'data'], check=True, shell=True)
        print("✓ Created data directory using subprocess")
    except Exception as e2:
        print(f"✗ Error with subprocess: {e2}")
        return

# Create file using basic file operations
try:
    csv_path = os.path.join('data', 'comprehensive_school_data.csv')
