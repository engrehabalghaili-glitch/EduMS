    ...
    IRequestHandler<DeleteAssetAssignmentCommand, bool>
{
    ...
    private readonly IGenericRepository<SchoolAsset> _assetRepository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IMapper _mapper;

    public AssetAssignmentCommandHandlers(
        IGenericRepository<AssetAssignment> repository,
        IGenericRepository<SchoolAsset> assetRepository,
    ...
    public async Task<long> Handle(CreateAssetAssignmentCommand request, CancellationToken cancellationToken)
    {
        ...
        var asset = await _assetRepository.GetByIdAsync(entity.AssetId, cancellationToken);
        if (asset != null)
        {
            // Assuming AssetStatusId 2 means 'Assigned' / 'In Use'
            asset.AssetStatusId = 2;
            await _assetRepository.UpdateAsync(asset, cancellationToken);
            
            entity.AddDomainEvent(new EduMS.Domain.Events.AssetAssignedDomainEvent(
                entity.AssetId,
                entity.AssigneeName,
