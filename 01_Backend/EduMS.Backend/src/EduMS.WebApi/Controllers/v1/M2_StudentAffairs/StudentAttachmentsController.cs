...
public class StudentAttachmentsController(MediatR.ISender sender) : ControllerBase
{
    ...
    {
        var result = await sender.Send(new GetAllStudentAttachmentsQuery());
    ...
    {
        var result = await sender.Send(new GetStudentAttachmentByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateStudentAttachmentCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateStudentAttachmentCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteStudentAttachmentCommand { Id = id });
