...
[Authorize]
public class SchoolSurplusesController(MediatR.ISender sender) : ControllerBase
{

    [HttpGet]
    public async Task<ActionResult<ApiResponse<IEnumerable<SchoolSurplusDto>>>> GetAll()
    {
        var result = await sender.Send(new GetAllSchoolSurplusesQuery());
    ...
    {
        var result = await sender.Send(new GetSchoolSurplusByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateSchoolSurplusCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateSchoolSurplusCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteSchoolSurplusCommand { Id = id });
