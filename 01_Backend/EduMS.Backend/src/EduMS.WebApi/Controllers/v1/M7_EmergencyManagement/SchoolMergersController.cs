...
[Authorize]
public class SchoolMergersController(MediatR.ISender sender) : ControllerBase
{

    [HttpGet]
    public async Task<ActionResult<ApiResponse<IEnumerable<SchoolMergerDto>>>> GetAll()
    {
        var result = await sender.Send(new GetAllSchoolMergersQuery());
    ...
    {
        var result = await sender.Send(new GetSchoolMergerByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateSchoolMergerCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateSchoolMergerCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteSchoolMergerCommand { Id = id });
