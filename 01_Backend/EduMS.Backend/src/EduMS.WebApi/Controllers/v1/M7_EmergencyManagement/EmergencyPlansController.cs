...
[Authorize]
public class EmergencyPlansController(MediatR.ISender sender) : ControllerBase
{

    [HttpGet]
    public async Task<ActionResult<ApiResponse<IEnumerable<EmergencyPlanDto>>>> GetAll()
    {
        var result = await sender.Send(new GetAllEmergencyPlansQuery());
    ...
    {
        var result = await sender.Send(new GetEmergencyPlanByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateEmergencyPlanCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateEmergencyPlanCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteEmergencyPlanCommand { Id = id });
