...
[Authorize]
public class EmergencyClosuresController(MediatR.ISender sender) : ControllerBase
{

    [HttpGet]
    public async Task<ActionResult<ApiResponse<IEnumerable<EmergencyClosureDto>>>> GetAll()
    {
        var result = await sender.Send(new GetAllEmergencyClosuresQuery());
    ...
    {
        var result = await sender.Send(new GetEmergencyClosureByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateEmergencyClosureCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateEmergencyClosureCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteEmergencyClosureCommand { Id = id });
