...
[Authorize]
public class EmergencyIncidentsController(MediatR.ISender sender) : ControllerBase
{

    [HttpGet]
    public async Task<ActionResult<ApiResponse<IEnumerable<EmergencyIncidentDto>>>> GetAll()
    {
        var result = await sender.Send(new GetAllEmergencyIncidentsQuery());
    ...
    {
        var result = await sender.Send(new GetEmergencyIncidentByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateEmergencyIncidentCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateEmergencyIncidentCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteEmergencyIncidentCommand { Id = id });
