...
[Authorize]
public class RemediationPlansController(MediatR.ISender sender) : ControllerBase
{

    [HttpGet]
    public async Task<ActionResult<ApiResponse<IEnumerable<RemediationPlanDto>>>> GetAll()
    {
        var result = await sender.Send(new GetAllRemediationPlansQuery());
    ...
    {
        var result = await sender.Send(new GetRemediationPlanByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateRemediationPlanCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateRemediationPlanCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteRemediationPlanCommand { Id = id });
