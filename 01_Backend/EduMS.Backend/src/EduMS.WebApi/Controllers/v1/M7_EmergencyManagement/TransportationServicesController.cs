...
[Authorize]
public class TransportationServicesController(MediatR.ISender sender) : ControllerBase
{

    [HttpGet]
    public async Task<ActionResult<ApiResponse<IEnumerable<TransportationServiceDto>>>> GetAll()
    {
        var result = await sender.Send(new GetAllTransportationServicesQuery());
    ...
    {
        var result = await sender.Send(new GetTransportationServiceByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateTransportationServiceCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateTransportationServiceCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteTransportationServiceCommand { Id = id });
