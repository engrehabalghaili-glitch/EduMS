...
[Authorize]
public class EmergencyHostingsController(MediatR.ISender sender) : ControllerBase
{

    [HttpGet]
    public async Task<ActionResult<ApiResponse<IEnumerable<EmergencyHostingDto>>>> GetAll()
    {
        var result = await sender.Send(new GetAllEmergencyHostingsQuery());
    ...
    {
        var result = await sender.Send(new GetEmergencyHostingByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateEmergencyHostingCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateEmergencyHostingCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteEmergencyHostingCommand { Id = id });
