...
[Authorize]
public class SubjectsController(MediatR.ISender sender) : ControllerBase
{
    ...
    {
        var result = await sender.Send(new GetSubjectByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateSubjectCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateSubjectCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteSubjectCommand { Id = id });
