...
public class AssetRevaluationImpairmentsController(MediatR.ISender sender) : ControllerBase
{
    ...
    {
        var result = await sender.Send(new GetAllAssetRevaluationImpairmentsQuery());
    ...
    {
        var result = await sender.Send(new GetAssetRevaluationImpairmentByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateAssetRevaluationImpairmentCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateAssetRevaluationImpairmentCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteAssetRevaluationImpairmentCommand { Id = id });
