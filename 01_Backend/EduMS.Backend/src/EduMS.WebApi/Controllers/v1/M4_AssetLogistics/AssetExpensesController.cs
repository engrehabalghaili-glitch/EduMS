...
public class AssetExpensesController(MediatR.ISender sender) : ControllerBase
{
    ...
    {
        var result = await sender.Send(new GetAllAssetExpensesQuery());
    ...
    {
        var result = await sender.Send(new GetAssetExpenseByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateAssetExpenseCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateAssetExpenseCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteAssetExpenseCommand { Id = id });
