...
public class EmployeeLeavesController(MediatR.ISender sender) : ControllerBase
{
    ...
    {
        var result = await sender.Send(new GetAllEmployeeLeavesQuery());
    ...
    {
        var result = await sender.Send(new GetEmployeeLeaveByIdQuery { Id = id });
    ...
    {
        var id = await sender.Send(new CreateEmployeeLeaveCommand { Dto = dto });
    ...
    {
        dto.Id = id;
        var result = await sender.Send(new UpdateEmployeeLeaveCommand { Dto = dto });
    ...
    {
        var result = await sender.Send(new DeleteEmployeeLeaveCommand { Id = id });
