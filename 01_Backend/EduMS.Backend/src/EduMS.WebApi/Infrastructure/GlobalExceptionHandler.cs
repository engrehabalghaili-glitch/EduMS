...
public class GlobalExceptionHandler(ILogger<GlobalExceptionHandler> logger) : IExceptionHandler
{
    ...
    private static ApiResponse<object> HandleUnauthorizedAccessException(HttpContext httpContext, UnauthorizedAccessException ex)
    {
        httpContext.Response.StatusCode = StatusCodes.Status403Forbidden;
        return ApiResponse<object>.Failure(ex.Message ?? "You are not authorized to perform this action.");
    }
}
