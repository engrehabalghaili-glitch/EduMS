...
type ExternalParticipationsStoreState = {
  ...
  selectedItem: ExternalParticipation | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: ExternalParticipationsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const ExternalParticipationsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(ExternalParticipationsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as ExternalParticipation] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ExternalParticipation[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ExternalParticipation[], isLoading: false });
            },
