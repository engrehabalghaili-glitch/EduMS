...
type EmployeePayrollFinancialContractsStoreState = {
  ...
  selectedItem: EmployeePayrollFinancialContract | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: EmployeePayrollFinancialContractsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const EmployeePayrollFinancialContractsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(EmployeePayrollFinancialContractsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as EmployeePayrollFinancialContract] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeePayrollFinancialContract[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as EmployeePayrollFinancialContract[], isLoading: false });
            },
