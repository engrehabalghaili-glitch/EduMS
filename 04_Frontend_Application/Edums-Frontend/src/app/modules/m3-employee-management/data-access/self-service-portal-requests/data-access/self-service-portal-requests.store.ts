...
import { SelfServicePortalRequest, CreateSelfServicePortalRequest, UpdateSelfServicePortalRequest } from '@modules/m3-employee-management/interfaces/self-service-portal-request.types';

type SelfServicePortalRequestsStoreState = {
  items: SelfServicePortalRequest[];
  selectedItem: SelfServicePortalRequest | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SelfServicePortalRequestsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SelfServicePortalRequestsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SelfServicePortalRequestsService)) => ({
    ...
    create: rxMethod<CreateSelfServicePortalRequest>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SelfServicePortalRequest] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateSelfServicePortalRequest}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SelfServicePortalRequest[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SelfServicePortalRequest[], isLoading: false });
            },
