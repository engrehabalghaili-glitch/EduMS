...
type TeacherSchedulesStoreState = {
  ...
  selectedItem: TeacherSchedule | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: TeacherSchedulesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const TeacherSchedulesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(TeacherSchedulesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as TeacherSchedule] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as TeacherSchedule[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as TeacherSchedule[], isLoading: false });
            },
