...
type ExamDistributionTimetablesStoreState = {
  ...
  selectedItem: ExamDistributionTimetable | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: ExamDistributionTimetablesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const ExamDistributionTimetablesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(ExamDistributionTimetablesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as ExamDistributionTimetable] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ExamDistributionTimetable[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as ExamDistributionTimetable[], isLoading: false });
            },
