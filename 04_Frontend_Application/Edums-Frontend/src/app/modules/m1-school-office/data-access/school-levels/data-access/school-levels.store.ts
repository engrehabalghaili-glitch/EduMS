...
import { SchoolLevel, CreateSchoolLevelDto, UpdateSchoolLevelDto } from '@modules/m1-school-office/interface/school-level';
...
export const SchoolLevelsStore = signalStore(
  ...
  withMethods((store, service = inject(SchoolLevelsService)) => ({
    ...
    create: rxMethod<CreateSchoolLevelDto>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
    ...
    update: rxMethod<{id: number | string, payload: UpdateSchoolLevelDto}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
