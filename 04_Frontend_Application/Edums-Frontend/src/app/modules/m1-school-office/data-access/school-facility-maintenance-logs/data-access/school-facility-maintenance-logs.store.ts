...
type SchoolFacilityMaintenanceLogsStoreState = {
  ...
  selectedItem: SchoolFacilityMaintenanceLog | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SchoolFacilityMaintenanceLogsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SchoolFacilityMaintenanceLogsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SchoolFacilityMaintenanceLogsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SchoolFacilityMaintenanceLog] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolFacilityMaintenanceLog[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolFacilityMaintenanceLog[], isLoading: false });
            },
