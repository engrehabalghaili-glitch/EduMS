...
type SchoolCanteenItemsStoreState = {
  ...
  selectedItem: SchoolCanteenItem | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: SchoolCanteenItemsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const SchoolCanteenItemsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(SchoolCanteenItemsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as SchoolCanteenItem] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolCanteenItem[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as SchoolCanteenItem[], isLoading: false });
            },
