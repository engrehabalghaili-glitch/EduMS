...
type AcademicLockPeriodsStoreState = {
  ...
  selectedItem: AcademicLockPeriod | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AcademicLockPeriodsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AcademicLockPeriodsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AcademicLockPeriodsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AcademicLockPeriod] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AcademicLockPeriod[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AcademicLockPeriod[], isLoading: false });
            },
