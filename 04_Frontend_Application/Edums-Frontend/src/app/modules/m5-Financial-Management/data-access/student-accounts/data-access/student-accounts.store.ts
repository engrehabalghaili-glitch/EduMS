...
import { StudentAccount, CreateStudentAccountDto, UpdateStudentAccountDto } from '@modules/m5-Financial-Management/interfaces/student-account.interface';

type StudentAccountsStoreState = {
  items: StudentAccount[];
  selectedItem: StudentAccount | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentAccountsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentAccountsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentAccountsService)) => ({
    ...
    create: rxMethod<CreateStudentAccountDto>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentAccount] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateStudentAccountDto}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentAccount[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentAccount[], isLoading: false });
            },
