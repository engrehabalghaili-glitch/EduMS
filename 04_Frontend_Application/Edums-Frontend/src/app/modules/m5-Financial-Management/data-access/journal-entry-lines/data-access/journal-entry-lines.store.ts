...
type JournalEntryLinesStoreState = {
  ...
  selectedItem: JournalEntryLine | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: JournalEntryLinesStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const JournalEntryLinesStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(JournalEntryLinesService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as JournalEntryLine] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as JournalEntryLine[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as JournalEntryLine[], isLoading: false });
            },
