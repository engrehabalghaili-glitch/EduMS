...
type PayrollRunsStoreState = {
  items: PayrollRun[];
  selectedItem: PayrollRun | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: PayrollRunsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const PayrollRunsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(PayrollRunsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as PayrollRun] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as PayrollRun[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as PayrollRun[], isLoading: false });
            },
