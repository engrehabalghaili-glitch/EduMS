...
import { AssetSuspensionRequest, CreateAssetSuspensionRequest, UpdateAssetSuspensionRequest } from '@modules/m4-assets-logistics/interfaces/asset-suspension-requests';

type AssetSuspensionRequestsStoreState = {
  items: AssetSuspensionRequest[];
  selectedItem: AssetSuspensionRequest | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetSuspensionRequestsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetSuspensionRequestsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetSuspensionRequestsService)) => ({
    ...
    create: rxMethod<CreateAssetSuspensionRequest>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetSuspensionRequest] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateAssetSuspensionRequest}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetSuspensionRequest[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetSuspensionRequest[], isLoading: false });
            },
