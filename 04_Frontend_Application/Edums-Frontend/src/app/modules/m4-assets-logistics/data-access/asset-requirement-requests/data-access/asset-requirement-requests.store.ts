...
import { AssetRequirementRequest, CreateAssetRequirementRequest, UpdateAssetRequirementRequest } from '@modules/m4-assets-logistics/interfaces/asset-requirement-requests';

type AssetRequirementRequestsStoreState = {
  items: AssetRequirementRequest[];
  selectedItem: AssetRequirementRequest | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AssetRequirementRequestsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AssetRequirementRequestsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(AssetRequirementRequestsService)) => ({
    ...
    create: rxMethod<CreateAssetRequirementRequest>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AssetRequirementRequest] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdateAssetRequirementRequest}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetRequirementRequest[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AssetRequirementRequest[], isLoading: false });
            },
