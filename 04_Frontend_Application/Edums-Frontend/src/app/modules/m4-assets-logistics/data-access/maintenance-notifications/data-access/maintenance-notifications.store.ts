...
type MaintenanceNotificationsStoreState = {
  ...
  selectedItem: MaintenanceNotification | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: MaintenanceNotificationsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const MaintenanceNotificationsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(MaintenanceNotificationsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as MaintenanceNotification] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as MaintenanceNotification[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as MaintenanceNotification[], isLoading: false });
            },
