...
type StudentExtracurricularAchievementsStoreState = {
  ...
  selectedItem: StudentExtracurricularAchievement | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentExtracurricularAchievementsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentExtracurricularAchievementsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentExtracurricularAchievementsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentExtracurricularAchievement] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentExtracurricularAchievement[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentExtracurricularAchievement[], isLoading: false });
            },
