...
type StudentFinancialAidApplicationsStoreState = {
  ...
  selectedItem: StudentFinancialAidApplication | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: StudentFinancialAidApplicationsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const StudentFinancialAidApplicationsStore = signalStore(
  { providedIn: 'root' },
  withState(initialState),
  withMethods((store, service = inject(StudentFinancialAidApplicationsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as StudentFinancialAidApplication] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentFinancialAidApplication[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as StudentFinancialAidApplication[], isLoading: false });
            },
