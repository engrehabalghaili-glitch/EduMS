...
import { Guardian, CreateGuardian, UpdateGuardian } from '@modules/m2-student-Affairs/interfaces/guardian.interface';
...
export const GuardiansStore = signalStore(
  ...
  withMethods((store, service = inject(GuardiansService)) => ({
    ...
    create: rxMethod<CreateGuardian>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
    ...
    update: rxMethod<{id: number | string, payload: UpdateGuardian}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
