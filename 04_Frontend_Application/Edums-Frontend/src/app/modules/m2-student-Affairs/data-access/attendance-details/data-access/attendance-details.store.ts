...
type AttendanceDetailsStoreState = {
  ...
  selectedItem: AttendanceDetail | null;
  isLoading: boolean;
  error: string | null;
};

const initialState: AttendanceDetailsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const AttendanceDetailsStore = signalStore(
  ...
  withMethods((store, service = inject(AttendanceDetailsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as AttendanceDetail] });
              }
        ...
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AttendanceDetail[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as AttendanceDetail[], isLoading: false });
            },
