...
import { Person, CreatePerson, UpdatePerson } from '@modules/m2-student-Affairs/interfaces/person.interface';

type PersonsStoreState = {
  items: Person[];
...
const initialState: PersonsStoreState = {
  items: [],
  selectedItem: null,
  isLoading: false,
  error: null,
...
export const PersonsStore = signalStore(
  ...
  withMethods((store, service = inject(PersonsService)) => ({
      ...
      pipe(
        ...
        switchMap((payload) => service.create(payload).pipe(
          tap({
            next: (response) => {
              if (response) {
                patchState(store, { items: [...store.items(), response as Person] });
              }
    ...
    update: rxMethod<{id: number | string, payload: UpdatePerson}>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap(({id, payload}) => service.update(id, payload).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Person[] });
              patchState(store, { isLoading: false });
            },
        ...
        switchMap((id) => service.delete(id).pipe(
          tap({
            next: () => {
              ...
              patchState(store, { items: updatedItems as Person[], isLoading: false });
            },
