...
### 5.3 خطة التنفيذ الفعلي لدورة المصادقة (Authentication Flow)
لتطبيق الحل الفعلي والكامل لمشكلة المصادقة (JWT) وربط الواجهات الأمامية بالخادم بأمان، سيتم بناء دورة مصادقة كاملة تتكون من 5 عناصر أساسية (توضع داخل مجلد `src/app/core/`):

1. **واجهات البيانات (Interfaces):** إنشاء ملف `auth.interface.ts` لتعريف شكل بيانات الإرسال `LoginRequest` (اسم المستخدم وكلمة المرور) وبيانات الاستقبال `LoginResponse` (الـ Token وصلاحيات المستخدم).
2. **خدمة المصادقة (`auth.service.ts`):** تحتوي على دوال الاتصال بالخادم `login()` لجلب التوكن وتخزينه بأمان في المتصفح، ودالة `getToken()` لاسترجاعه، ودالة `logout()` لحذفه وطرد المستخدم.
3. **المعترض (`api.interceptor.ts`):** حارس البوابة الذي يراقب كل طلب HTTP يخرج من الواجهة، يسحب التوكن من الـ Service ويحقنه في الترويسة `Authorization: Bearer <TOKEN>`. وفي حال انتهاء التوكن (401 Unauthorized) يقوم بتحويل المستخدم لصفحة الدخول.
4. **حارس المسارات (`auth.guard.ts`):** يحمي الروابط من الدخول المباشر والتأكد من امتلاك المستخدم لتوكن صالح قبل عرض الشاشة.
5. **شاشة تسجيل الدخول (Login Component):** شاشة الواجهة المرئية لجمع بيانات الدخول وإرسالها لـ `auth.service.ts`. *(ملاحظة: يمكن مؤقتاً برمجة دخول تلقائي Auto-login ببيانات ثابتة في الخدمة لتسريع التطوير وتجاوز الشاشة مؤقتاً)*.
...
## 18. التوافق والربط المباشر مع الباك إند (Integration & Compatibility)

لضمان الاتصال الصحيح والسلس بين الواجهة الأمامية المبنية بـ Angular والواجهة الخلفية المبنية بـ .NET (Clean Architecture)، تم تطبيق معيارين أساسيين في بنية المشروع:

### 18.1 توجيه الروابط (API Proxy Configuration)
الواجهة الأمامية تعتمد على مسارات نسبية (Relative Paths) مثل `/api/v1/...` للاتصال بالخادم. لكي تعمل هذه المسارات في بيئة التطوير (Development) دون التسبب بخطأ `404 Not Found` أو أخطاء `CORS`، تم اعتماد آلية **Proxy**:
1. **ملف `proxy.conf.json`**: موجود في جذر المشروع ويوجه أي طلب يبدأ بـ `/api` إلى خادم الباك إند الحقيقي (مثلاً: `https://localhost:7145`).
2. **إعدادات `angular.json`**: تم دمج البروكسي ضمن إعدادات `serve -> options -> proxyConfig`. 
- **القاعدة:** يُمنع تماماً كتابة الرابط الكامل للباك إند (Hardcoding URLs) داخل الكود، بل يُعتمد على البروكسي محلياً، وعلى الـ Interceptor و الـ Environment في بيئة الإنتاج (Production).

### 18.2 التعامل مع غلاف البيانات (ApiResponse Wrapper)
جميع استجابات الباك إند تأتي مغلفة بكلاس `ApiResponse<T>` المعياري (الذي يحتوي على `succeeded`, `message`, `data`, `errors`). 
لتفادي انهيار الجداول ومكونات الواجهة التي تتوقع البيانات صافية (مثل مصفوفة `T[]`)، تم حل هذا التعارض في **نقطة مركزية واحدة**:
- **كلاس `BaseApiService`**: هو العقل المدبر المركزي. تم تحديثه ليستقبل `ApiResponse<T>` ويقوم بفك الغلاف آلياً باستخدام `pipe(map(res => res.data))` قبل تمرير النتيجة لبقية أجزاء التطبيق (Stores و Components).
- **النتيجة:** مكونات التطبيق (UI) ومديرو الحالة (Signals) يتعاملون مع البيانات الصافية مباشرة دون الحاجة لمعرفة كيف تمت هيكلة استجابة السيرفر.
